...
/* ── Verify GitHub contribution (AJAX) ── */
function initVerifyContribution() {
  document.querySelectorAll('.btn-verify-contribution').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.preventDefault();
      const issueId = btn.getAttribute('data-issue-id');
      const csrfToken = getCookie('csrftoken');
      ...
      try {
        ...
        const response = await fetch(`/issues/${issueId}/verify-contribution/`, {
          method: 'POST',
          headers: {
            'X-CSRFToken': csrfToken,
            'Content-Type': 'application/json',
        ...
        if (response.ok && data.status === 'solved') {
          showToast('Verified merged pull request. Marked as solved.', 'success');
        ...
        showToast(data.message || 'No merged pull request found yet.', 'info');
      } catch {
        btn.disabled = false;
        showToast('An error occurred. Please try again.', 'danger');
      }
